...
const CardActor = ({ actor }: CardActorProps) => {
  ...
  const [actImg, setActImg] = useState<boolean>(false);
  ...
  return(
    <article className='w-28 h-fit'>
      <div className='h-14 w-14 relative lg:h-20 lg:w-20'>
        {!actImg && <SkeletonActors/>}
        <Image src={webpImgSrc} alt={`Foto de ${actor.name}`} fill={true} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className='rounded-full object-cover' onLoad={handleActImg}/>
      </div>
