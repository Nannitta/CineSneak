    ...
    <div>
      {
        isMinimize
          ? <div onClick={handleClose} onTouchStart={handleClose}
              className={`${screenSize === "sm" && !isHover ? "fixed bottom-[250px] right-0 bg-black z-20" : (screenSize === 'md' && !isHover) ? 'fixed bottom-[300px] right-0 bg-black' : (screenSize === "lg" && isHover ) ? "items-center justify-center fixed bottom-[89px] right-[144px] z-20 md:bottom-[114px] md:right-[214px] lg:bottom-[89px] lg:right-[189px] lg:bg-black lg:bg-opacity-40 lg:rounded-full lg:p-2" : "hidden"}`}
      ...
      <aside className={`${movieTrailer === "" || isInPictureMode === false ? "hidden" : (isMinimize ? "flex fixed bottom-0 right-0 z-10" : "w-full h-screen flex fixed inset-0 z-10 bg-black bg-opacity-80 items-center justify-center")}`} onClick={minimizePictureMode}>
