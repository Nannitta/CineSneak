describe('Open and close view trailer', () => {
  ...
  it('Open player when click on button trailer', () => {   
    ...
    const player: Cypress.Chainable<JQuery<HTMLElement>> = cy.get('aside > div > iframe')
      .should('have.attr', 'src')
      .and('include', 'autoplay=1')
      .and('include', 'youtube.com/embed/');

    const asidePlayer: Cypress.Chainable<JQuery<HTMLElement>> = cy.get('aside');
    asidePlayer.click();
  ...
  it('Close player when click on close button' , () => {
    ...
    const closePlayerButton: Cypress.Chainable<JQuery<HTMLElement>> = cy.get('div[data-test="closePlayer"]');
    closePlayerButton.should('have.class', 'hidden');

    const containerPlayer: Cypress.Chainable<JQuery<HTMLElement>> = cy.get('div[data-test="containerCloseButton"]');
